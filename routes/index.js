...
router.get('/', function(req, res, next) {
    // task_col is a database object variable
    req.task_col.find({completed:false}).toArray(function(err, tasks){
        if (err) {
            return next(err);
        }
        res.render('index', { title: 'TODO list' , tasks: tasks }); // directs to index.hbs
    });
});
...
router.get('/completed', function(req, res, next){
    req.task_col.find({completed:true}).toArray(function(err, tasks){
        ...
        res.render('tasks_completed', { title: 'Completed tasks' , tasks: tasks }); // directs to task_completed page
    });
});
